...
const ItemReadySection: React.FC<ItemReadySectionProps> = ({
  ...
  return (
    <div className="bg-gradient-to-r from-emerald-100 via-teal-100 to-cyan-100 p-6 rounded-2xl border-2 border-emerald-200 shadow-lg backdrop-blur-sm relative overflow-hidden hover:shadow-xl transition-all duration-300">
      ...
      <div className="flex items-center space-x-6 group relative z-10">
        <div className="flex items-center space-x-4">
          <Checkbox
            id="isReady"
            checked={isReady}
            onCheckedChange={(checked) => setIsReady(!!checked)}
            className="border-2 border-emerald-600 data-[state=checked]:bg-emerald-600 data-[state=checked]:border-emerald-600 transition-all duration-300 hover:scale-110 w-7 h-7 rounded-md shadow-md"
          />
          <Label 
            htmlFor="isReady" 
